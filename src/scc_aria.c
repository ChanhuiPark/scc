				...
				const int dir)
{
	...
	if (keyLength < SC_ARIA_MIN_KEY_SIZE) {
	...
	cipherDir = (dir==SC_ARIA_DIR_ENCRYPT)?(SC_ARIA_DIR_ENCRYPT):(SC_ARIA_DIR_DECRYPT);
	...
	if (cipherDir == SC_ARIA_DIR_DECRYPT) {
		for (j = 0; j < 16; j++) {
			t[j] = rk[j];
			rk[j] = rk[16*R + j];
			rk[16*R + j] = t[j];
						   ...
						   const U32 blockSize)
{
	...
	case SCC_CIPHER_PADDING_HASH :
		i = 0;
		while ((input[(inputLength -1) - i] == 0x00) && (i < blockSize -1))
			i++;
...
static
int
SC_ARIA_ECB(SC_ARIA_CONTEXT *ctx,
			  U8 *output,
			  U32 *outputLength,
			  const U8 *input,
			  const U32 inputLength,
			   ...
			   const int dir)
{
	...
	case SCC_CIPHER_MODE_CBC :
		...
		break;
	default:
		retCode = SCC_CIPHER_ERROR_UNKNOWN_ID;
	}
					  ...
					  U32 *outputLength)
{
	...
	if (ctx->paddingID == SCC_CIPHER_PADDING_NO) {
		if (ctx->remainLength) {
			retCode = SCC_ARIA_ERROR_ENCRYPT_FAILED;
			goto end;
		}
				  ...
				  const int paddingID)
{
	...
	if (keyLength < SC_ARIA_MIN_KEY_SIZE || keyLength > SC_ARIA_MAX_KEY_SIZE){
		retCode = SCC_ARIA_ERROR_KEY_LENGTH;
		goto end;
	}

	if (ivLength != SC_ARIA_IV_SIZE && ivLength != 0){
	...
	*outputLength += length;
	...
	// 키제로화
	SC_Memzero(&ariaCTX, 0, sizeof(SC_ARIA_CONTEXT));
					   ...
					   const U32 paddingID)
{
	...
	if (ivLength != SC_ARIA_IV_SIZE && ivLength != 0){
		return SCC_ARIA_ERROR_IV_LENGTH;
	}
				...
				const int paddingID)
{
	...
	if ((keyLength < SC_ARIA_MIN_KEY_SIZE) && (ivLength != 0)){
		retCode = SCC_ARIA_ERROR_KEY_LENGTH;
		goto end;
	}
	...
	retCode = SC_ARIA_Decrypt_Update(&ariaCTX, output, outputLength, 
										 input, inputLength);
	if (retCode != 0) goto end;
